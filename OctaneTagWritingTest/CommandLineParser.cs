public static class CommandLineParser
{
        public static ApplicationConfig ParseArgs(string[] args)
        {
            ...
            // NOVOS PARÂMETROS: Configuração GPI
            string useGpiForVerification = GetArgumentValue(args, "--use-gpi-verification");
            if (!string.IsNullOrEmpty(useGpiForVerification) && bool.TryParse(useGpiForVerification, out bool useGpi))
                config.UseGpiForVerification = useGpi;

            string gpiTriggerState = GetArgumentValue(args, "--gpi-trigger-state");
